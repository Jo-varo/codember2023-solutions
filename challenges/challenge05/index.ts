const toBeSolved = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
export function checkInvalidRowsDB(list: string) {
  const listArr = list
    .split('\n')
    .map((row) => row.trim())
    .filter((row) => row !== '');
  ...
  const isValidRow = (row: string) => {
    ...
    const checkData = {
      id_username: (txt: string) => txt !== '' && regexes.alfa.test(txt),
      email: (email: string) => email !== '' && regexes.email.test(email),
      age: (age: string) => regexes.age.test(age) && Number(age) < 120,
      location: (location: string) => typeof location === 'string',
    };

    const columns = row.split(',');
    if (columns.length !== 5) return false;

    return columns.every((data, i) => {
      if (i === 0 || i === 1) return checkData.id_username(data);
      if (i === 2) return checkData.email(data);
      if (i === 3) return checkData.age(data);
      if (i === 4) return checkData.location(data);
      return false;
    });
  };
  ...
  return listArr
    ...
    .map((row) => ({ username: row.split(',')[1], input: row }));
}

console.log(checkInvalidRowsDB(toBeSolved));
