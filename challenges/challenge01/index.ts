...
export function findPatterns(word: string) {
  const results: { [key: string]: number } = {};
  word
    .toLocaleLowerCase()
    .split(' ')
    .map((e) => {
      results[e] ? (results[e] = results[e] + 1) : (results[e] = 1);
    });

  let result = '';
  const objectResults = Object.entries(results);
  objectResults.forEach(([k, v]) => {
    result += `${k}${v}`;
  });
...
console.log(findPatterns(toBeSolved));
