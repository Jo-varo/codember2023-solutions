const toBeSolved = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
//Returns a list of the invalid passwords
export function passwordsChecker(list: string) {
  const isAValidPass = (checkPass: string) => {
    checkPass = checkPass.trim();
    const [rule, pass] = checkPass.split(': ');
    const [minN, maxN, letter] = [rule[0], rule[2], rule.slice(-1)];
    let charCounter = 0;
    pass.split('').map((char) => {
      if (char === letter) charCounter++;
    });

    return charCounter >= Number(minN) && charCounter <= Number(maxN);
  };

  let res: string[] = [];
  let arrRes = list.split('\n');

  arrRes.map((listItem) => {
    listItem = listItem.trim();
    if (!isAValidPass(listItem)) res.push(listItem);
  });

  return Object.assign({}, res);
}

console.log(passwordsChecker(toBeSolved))